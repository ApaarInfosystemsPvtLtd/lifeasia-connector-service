...
@Component
public class MQClientConnector {

    @Autowired
    MasterStore masterStore;

    MQQueueManager mqQueueManager;
    public static final String EXCEPTION_IN_MQCLIENT_CONNECTOR = "Exception in MQClientConnector1::{}";
    private Logger logger = LoggerFactory.getLogger(MQClientConnector.class);
    MQClientConnector mqClientConnector = null;

    public MQClientConnector() {
        try {
            MQEnvironment.properties.put(CMQC.TRANSPORT_PROPERTY, CMQC.TRANSPORT_MQSERIES_CLIENT);
        } catch (Exception e) {
            logger.error(Marker.ANY_MARKER, EXCEPTION_IN_MQCLIENT_CONNECTOR, e);
        }
    }

    public MQClientConnector(String host, int port, String user, String password, String channelName) {
        MQEnvironment.properties.put(CMQC.TRANSPORT_PROPERTY, CMQC.TRANSPORT_MQSERIES_CLIENT);
    ...
    public MQClientConnector getInputMQ() {
        logger.info("getInputMQ::propertyReader::{}",masterStore.toJson());
        try {
            mqClientConnector = new MQClientConnector(masterStore.getConnectionMap().get("host"),
        ...
        return mqClientConnector;
    }

    public MQClientConnector getOutputMQ() {
    ...
    public String sendMessage(String message) {
        byte[] output = null;
        try {
            MQClientConnector manager = getInputMQ();
    ...
    public String receiveMessage(byte[] bts) {
        String msg = "";
        MQClientConnector mqClient = getOutputMQ();
        try {
